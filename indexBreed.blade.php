...
@section('content')
    ...
    <body>
        <div class="container mt-2">
            <div class="header">
                <h2>ทะเบียนข้อมูลการผสมพันธุ์</h2>
            </div>
                        ...
                        <tbody>                       
                            @foreach ($breed as $breed)
                                <tr>
                                    <td>{{ $breed->goat_id }}</td>                                    
                                    <td>{{ $breed->breedNo }}</td>
                                    <td>{{ $breed->dateOfBreed }}</td>
                                    <td>{{ $breed->father_breeder }}</td>
                                    <td>{{ $breed->breed_staff }}</td>
                                    <td>
                                        <form action="{{ route('goats.destroyBreed',$breed->breedId) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
