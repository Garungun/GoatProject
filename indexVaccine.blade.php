...
@section('content')
    ...
    <body>
        <div class="container mt-2">
            <div class="header">
                <h2>ทะเบียนข้อมูลการฉีดวัคซีน</h2>
            </div>
                    ...
                    <table id="goats">
                        ...
                        <tbody>
                            @foreach ($vaccine as $vaccine)
                                <tr>
                                    <td>{{ $vaccine->goat_id }}</td>                                    
                                    <td>{{ $vaccine->typeOfVaccine }}</td>
                                    <td>{{ $vaccine->dateOfVaccine }}</td>
                                    <td>{{ $vaccine->vaccine_staff }}</td>
                                    <td>
                                        <form action="{{ route('goats.destroyVaccine',$vaccine->vaccineId) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
